...
char *col_hidden;
char *row_hidden;
char ExecLine[2048]; // should be enough for any command line =)
char RefreshInterval[48]; // idem
char line[FBUFLEN];
int changed;
...
char *slatexext;
char *texext;
char *tmpfilename ="/dev/shm/sc.tmp"; // 
int dbgcalls = 0; // TODO: remove. counts number of times sheet has been forcedly refreshed
int scrc = 0;
...
handler(int sig, siginfo_t *si, void *uc)
{
    ...
    char outp[FBUFLEN];           
     ...
     dbgcalls++; // not really needed
	 FILE *pp;
	 (void) sprintf(buff, "%s > %s", ExecLine, tmpfilename); /* build cmd line */
			//fprintf(stdout,"executing: %s\n",buff);
			system(buff);
			if ((c = open(tmpfilename, O_RDONLY)) > -1) {
					close(c);
					(void) readfile(tmpfilename, 0);
					}
           //print_siginfo(si);
           //signal(sig, SIG_IGN); // deletes timer
       }
//// timer       
...
main (int argc, char  **argv)
{
    ...
    while ((c = getopt(argc, argv, "axmoncrCReP:W:vqX:T:")) != EOF) {
    	switch (c) {
	    case 'a':
		    skipautorun = 1;
		    break;
	    ...
	    case 'c':
		    ...
		    break;
	    case 'X':
	        strcpy(ExecLine,optarg);
	        fprintf(stderr, "ExecLine : '%s'\n",ExecLine);
	        slavemodeopt = 1;
		    Xmode = 1; // redundant identifier is redundant
		    break;
	    case 'T':
	        strcpy(RefreshInterval,optarg);
	        fprintf(stderr, "Refresh Interval : '%s'\n",RefreshInterval);
		    Tmode = 1;
		    break;			    		    
	    case 'r':
		    ropt = 1;
	    ...
	    default:
		    (void) fprintf(stderr,
			    "Usage: %s [-acemnoqrxCR] [-X cmdline] [-T secs] [file...]\n", progname);
		    exit (1);
	}
    }
...
// timer related stuff + paste leftovers

           timer_t timerid;
           struct sigevent sev;
           struct itimerspec its;
           long long freq_nanosecs=10000000000LL;
           sigset_t mask;
           struct sigaction sa;
	...
	if (slavemodeopt) {
           ...
           sev.sigev_notify = SIGEV_NONE; // SIGEV_SIGNAL;
           sev.sigev_notify = SIGEV_SIGNAL; // TODO
           sev.sigev_signo = SIG;
          ...
          if (RefreshInterval[0] == (char)0)  // is it empty?
			{
			//fprintf(stdout,"Refresh interval is empty, defaulting to 5 seconds.");
			freq_nanosecs = atoll("500000000");
			}
          else
			{
     		freq_nanosecs = atoll(RefreshInterval)*1000000000LL;
            }

			//fprintf(stdout,"Refresh interval is %s \n",RefreshInterval);
			//fprintf(stdout,"freq_nanosecs is %ld \n", freq_nanosecs);
 ...
 */
		...
		case -1:
		   // you have been interrupted by a signal call but this shouldnt happen in reality so nevermind, go on.
		   break;
...
doquit()
{
    ...
    write_hist();
    fprintf(stdout,"calls: %d",dbgcalls);
    fprintf(stdout,"BYE");
    exit (1);
}
